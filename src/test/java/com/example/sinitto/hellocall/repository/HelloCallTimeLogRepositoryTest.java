...
class HelloCallTimeLogRepositoryTest {
    ...
    void findBySinittoAndHelloCallIdTest() {
        HelloCallTimeLog timeLog = new HelloCallTimeLog(helloCall, sinittoBankInfo, LocalDateTime.of(2024, 1, 1, 10, 0), LocalDateTime.of(2024, 1, 1, 11, 0));
        helloCallTimeLogRepository.save(timeLog);

        Optional<HelloCallTimeLog> foundTimeLog = helloCallTimeLogRepository.findBySinittoAndHelloCallId(sinittoBankInfo, helloCall.getId());

        assertThat(foundTimeLog).isPresent();
        assertThat(foundTimeLog.get().getHelloCall()).isEqualTo(helloCall);
        assertThat(foundTimeLog.get().getSinitto()).isEqualTo(sinittoBankInfo);
    ...
    void findAllByHelloCallIdTest() {
        HelloCallTimeLog timeLog1 = new HelloCallTimeLog(helloCall, sinittoBankInfo, LocalDateTime.of(2024, 1, 1, 10, 0), LocalDateTime.of(2024, 1, 1, 11, 0));
        HelloCallTimeLog timeLog2 = new HelloCallTimeLog(helloCall, sinittoBankInfo, LocalDateTime.of(2024, 1, 1, 12, 0), LocalDateTime.of(2024, 1, 1, 13, 0));
    ...
    void findTopBySinittoAndHelloCallOrderByStartDateAndTimeDescTest() {
        ...
        assertThat(recentTimeLog.get().getSinitto()).isEqualTo(sinittoBankInfo);
